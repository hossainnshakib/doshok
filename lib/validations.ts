...
export const checkoutSchema = z.object({
  ...
  districtId: z.string().optional().nullable(),
  districtName: z.string().min(1).max(100).transform((v) => v.trim()),
  upazilaName: z.string().min(1).max(100).transform((v) => v.trim()),
  areaName: z.string().max(100).optional().transform((v) => v?.trim() || ""),
