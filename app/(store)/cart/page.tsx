  ...
  return (
    <div className="container mx-auto container-px py-8 md:py-12">
      <div className="flex items-center justify-between mb-8">
        <Link href="/products" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1.5 transition-colors group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <Card key={`${item.productId}-${item.variantId}`} className="overflow-hidden border-border/50 rounded-2xl shadow-sm">
              <CardContent className="flex gap-4 md:gap-6 p-4 md:p-5">
                {item.image && (
                  <div className="w-24 h-24 md:w-28 md:h-28 bg-muted rounded-xl overflow-hidden shrink-0">
                    <img
                      src={item.image}
                      alt={item.name}
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex items-center gap-3">
                      <p className="font-semibold text-sm md:text-base">৳{(item.price * item.quantity).toLocaleString()}</p>
                      <button
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <Card className="border-border/50 rounded-2xl shadow-sm">
            <CardContent className="p-6 md:p-8 space-y-5">
              ...
              <Link
                href="/checkout"
                className="flex h-12 items-center justify-center rounded-xl bg-primary text-primary-foreground text-sm font-medium w-full hover:bg-primary/90 transition-all hover:scale-[1.01] active:scale-[0.99]"
              >
                Proceed to Checkout
              </Link>
            </CardContent>
