...
export default function NewProductPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/size-charts")
      .then((r) => r.json())
      .then((d) => { if (d.success) setAllSizeCharts(d.data.map((c: { id: string; name: string }) => ({ id: c.id, name: c.name }))) })
      .catch(() => {})
  }, [])
          ...
          <AdminSectionCard title="Product Images">
            <div className="space-y-2">
              <ImageUploader
                ...
                folder="products"
              />
              {noImages && <p className="text-[11px] text-amber-500 flex items-center gap-1"><AlertTriangle className="h-3 w-3" /> No images yet — add at least one</p>}
            </div>
          </AdminSectionCard>
        ...
        <div className="space-y-4">
          <div className="sticky top-6 space-y-4">
            ...
            <div className="rounded-xl border border-slate-200/60 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2">
                <Button type="button" disabled={loading} onClick={() => handleSubmit("Draft")} variant="outline" className="w-full h-9 rounded-lg justify-start gap-2 text-xs font-semibold">
