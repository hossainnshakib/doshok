          ...
          <AdminSectionCard title="Product Images">
            <div className="space-y-2">
              <ImageUploader
                ...
                folder="products"
              />
              {noImages && <p className="text-xs text-amber-600 flex items-center gap-1"><AlertTriangle className="h-3 w-3" /> No images yet</p>}
            </div>
          </AdminSectionCard>
        ...
        <div className="space-y-4">
          <div className="sticky top-6 space-y-4">
            ...
            <div className="rounded-[1.5rem] border border-black/5 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-2">
                <Button type="button" disabled={loading} onClick={() => handleSubmit("Draft")} variant={status === "Draft" ? "default" : "outline"} className="w-full h-10 rounded-full justify-start gap-2">
            ...
            {slug && (
              <Link
                href={previewUrl}
                target="_blank"
                className="inline-flex items-center justify-center gap-2 w-full h-11 rounded-full border border-black/10 bg-white text-sm font-medium hover:bg-neutral-50 transition-colors shadow-sm"
              >
